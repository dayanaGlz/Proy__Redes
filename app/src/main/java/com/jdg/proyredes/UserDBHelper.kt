...
class UsersDBHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,
    ...
    fun insertUser(user: UserModel): Boolean {
        ...
        values.put(DBContract.UserEntry.COLUMN_TELEF, user.telef)
    ...
    fun readUser(correo: String, contra: String): Boolean {
        ...
        if (cursor!!.moveToFirst()) {
            while (cursor.isAfterLast == false) {
                ...
                telef = cursor.getString(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_TELEF))

                users.add(UserModel(correo, contra, telef))
                cursor.moveToNext()
            }
        }
        return true
